...
function LoginContent() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // The cart is stored in localStorage of quetz.org, but Google OAuth goes through
      // Railway's domain, so we need a backup in a cookie that persists across the redirect
      if (items.length > 0) {
      ...
      // Also save redirect destination
      ...
      >
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600 text-sm">{t('auth.login')}</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={loading}
              className={`w-full bg-quetz-green text-white font-semibold py-3 px-4 rounded-xl hover:bg-green-700 transition-colors flex items-center justify-center gap-2 disabled:opacity-50 ${isRTL ? 'flex-row-reverse' : ''}`}
            >
              {loading ? (
                <>
          ...
          <p className="text-center mt-6 text-gray-600">
            {t('auth.noAccount')}{' '}
            <Link href={`/auth/registro${redirectTo !== '/mi-bosque' ? `?redirect=${encodeURIComponent(redirectTo)}` : ''}`} className="text-quetz-green font-semibold hover:underline">
              {t('auth.registerHere')}
            </Link>
          </p>
        </div>
